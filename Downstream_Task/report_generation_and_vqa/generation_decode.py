...
def main():
    ...
    parser.add_argument("--config_path", default='output_model/config.json', type=str, help="Bert config file path.")
    parser.add_argument("--model_recover_path", default='output_model/model.50.bin', type=str,
                        help="The file of fine-tuned pretraining model.") # model load
    parser.add_argument('--src_file', default='../../data/open_i/Test.jsonl', type=str)
    ...
    # print(args.model_recover_path.strip())
    # print("path",glob.glob("output_model/model.5.bin"),"MKC",glob.glob(args.model_recover_path.strip()))
    ...
    for model_recover_path in glob.glob(args.model_recover_path.strip()):
        ...
        for bootstrap in range(1,args.random_bootstrap_testnum+1):
            ...
            eval_lst = []
            # print(args.src_file)
            # print(11,os.listdir(os.getcwd()+"/../../data/openi"))
            # CHANGES
            img_dat = [json.loads(l) for l in open(os.getcwd()+"/../../data/openi/Test.jsonl")]
            ...
            if args.teacher_forcing:
                model.train()
                for i_epoch in trange(1, 2, desc="Epoch"):
                ...
                with tqdm(total=total_batch) as pbar:
                    while next_i < len(input_lines):
                        ...
                        instances = []
                        for instance in [(buf[x], args.len_vis_input, gt_report[x]) for x in range(2)]:
                            for proc in bi_uni_pipeline:
                                instances.append(proc(instance))
