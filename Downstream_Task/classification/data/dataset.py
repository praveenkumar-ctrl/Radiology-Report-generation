...
class JsonlDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.data[index]["img"]:
            paths= self.data[index]["img"]
            path1=os.path.split(paths)
            path2=os.path.split(path1[0])
            # print('-----',index,os.path.join(self.data_dir, self.data[index]["img"]))
            # image = Image.open(os.path.join(self.data_dir, self.data[index]["img"]))
            image = Image.open(os.path.join("/home/cse/ckm/code/seelam/code/MedViLL/data/home/data_storage/mimic-cxr/dataset/image_preprocessing/re_512_3ch",path2[1],path1[1]))
        else:
