...
class CXRBERT_Trainer():
    def __init__(self, args, train_dataloader, test_dataloader=None):
        ...
        if args.weight_load:
            ...
            # config = AutoConfig.from_pretrained(args.pre_trained_model_path)
            # path2="/home/cse/ckm/code/seelam/code/MedViLL/home/cxr-bert/par/pytorch_model.bin"
            # model_state_dict = torch.load(path2)
            # path1="/home/cse/ckm/code/seelam/code/MedViLL/home/cxr-bert/par/config.json"
            # self.model = CXRBERT.from_pretrained(path1, state_dict=model_state_dict,
            #                                      config=config, args=args).to(self.device)
            print('resume')
            print(config)
        else:
